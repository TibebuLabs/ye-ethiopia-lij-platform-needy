        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-slate-50">
              <tr>
                <th className="text-left p-4 text-sm font-semibold text-slate-700">Child</th>
                <th className="text-left p-4 text-sm font-semibold text-slate-700">Organization</th>
                <th className="text-left p-4 text-sm font-semibold text-slate-700">Submitted</th>
                <th className="text-left p-4 text-sm font-semibold text-slate-700">Status</th>
                <th className="text-left p-4 text-sm font-semibold text-slate-700">Reviewed</th>
                ...
                <tr key={submission.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors group">
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <p className="font-medium text-slate-900 group-hover:text-[#2E8B57] transition-colors">
                  ...
                  <td className="p-4">
                    <p className="text-sm text-slate-700">{submission.organization}</p>
                    <p className="text-xs text-slate-500">{submission.location}</p>
                  </td>
                  <td className="p-4">
                    <p className="text-sm text-slate-600">{submission.submittedDate}</p>
                  </td>
                  <td className="p-4">
            ...
            <div className="space-y-4">
              <div className="bg-slate-50 p-4 rounded-xl">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  />
                  <div>
                    <p className="font-semibold text-slate-900">{selectedSubmission.childName}</p>
                    <p className="text-xs text-slate-500">{selectedSubmission.organization}</p>
                  </div>
                </div>
                
                <p className="text-sm text-slate-600 mb-2">
                  <span className="font-medium text-slate-700">Bio:</span> {selectedSubmission.bio}
                </p>
              </div>

              {selectedSubmission.feedback && (
                <div className="p-4 rounded-xl border border-red-200 bg-red-50">
                  <p className="text-sm font-medium text-red-700 mb-2">Feedback:</p>
                  <p className="text-sm text-red-600">{selectedSubmission.feedback}</p>
      ...
      {filteredSubmissions.some(s => s.status === 'rejected' && s.feedback) && !showFeedbackModal && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 animate-pulse">
          <div className="flex items-start gap-3">
            <WarningIcon className="text-amber-600 mt-1" />
            <div>
              <h4 className="font-semibold text-amber-800 mb-2">Rejection Feedback Available</h4>
